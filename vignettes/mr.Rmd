...
```{r}
if (!file.exists("ieu-a-300.vcf.gz")) {
  download.file(
    url = "https://gwas.mrcieu.ac.uk/files/ieu-a-300/ieu-a-300.vcf.gz",
    destfile = "ieu-a-300.vcf.gz")
} 

if (!file.exists("ieu-a-300.vcf.gz.tbi")) {
  download.file(
    url = "https://gwas.mrcieu.ac.uk/files/ieu-a-300/ieu-a-300.vcf.gz.tbi",
    destfile = "ieu-a-300.vcf.gz.tbi")
}

if (!file.exists("ieu-a-7.vcf.gz")) {
  download.file(
    url = "https://gwas.mrcieu.ac.uk/files/ieu-a-7/ieu-a-7.vcf.gz",
    destfile = "ieu-a-7.vcf.gz")
}

if (!file.exists("ieu-a-7.vcf.gz.tbi")) {
  download.file(
    url = "https://gwas.mrcieu.ac.uk/files/ieu-a-7/ieu-a-7.vcf.gz.tbi",
    destfile = "ieu-a-7.vcf.gz.tbi")
}
```
...
```{r}
outd2 <- gwasvcf::query_gwas("ieu-a-7.vcf.gz", 
                             chrompos = paste0(expd1$chr.exposure, 
                                               ":", expd1$pos.exposure))
...
```{r}
if (any(!file.exists(c("data_maf0.01_rs_ref.bed", 
                       "data_maf0.01_rs_ref.bim", 
                       "data_maf0.01_rs_ref.fam")))) {
  if (!file.exists("data_maf0.01_rs_ref.tgz")) {
    download.file(
      url = "http://fileserve.mrcieu.ac.uk/ld/data_maf0.01_rs_ref.tgz",
      destfile = "data_maf0.01_rs_ref.tgz"
    )
  }

  untar(tarfile = "data_maf0.01_rs_ref.tgz")
}
```
...
- [MR-RAPS](https://cran.r-project.org/package=mr.raps)
