...
#' @param threshold Default=`0.9` If the proportion of allele strands match is 
                              ...
                              ref_a2, threshold=0.9)
{
	...
	g <- dplyr::data_frame(SNP=gwas_snp, A1=toupper(gwas_a1), 
	                       A2=toupper(gwas_a2)) %>%
...
#' @param strand_flip default=`FALSE`. If `TRUE` then attempts to flip strand 
#' when alignment is not otherwise possible
#'
#' @export
...
{
	chr <- pos <- ref <- alt <- decision <- NULL # Fix for R CMD check note
