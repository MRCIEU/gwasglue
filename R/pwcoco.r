                              ...
                              outfile)
{
	...
	if (type1 == "quant" || VariantAnnotation::header(vcf1) %>% 
                               ...
                               outfile)
{
	...
	write.table(tib1, file=paste0(outfile, "1.txt"), row.names = FALSE, 
	            col.names = TRUE, quote = FALSE)
	write.table(tib2, file=paste0(outfile, "2.txt"), row.names = FALSE, 
	            col.names = TRUE, quote = FALSE)
	return(0)
}
                   ...
                   workdir=tempdir())
{
	if (file.exists(id1) && file.exists(id2))
	{
		...
		stopifnot(gwasvcf_to_pwcoco(id1, id2, chrompos, type1, type2, 
